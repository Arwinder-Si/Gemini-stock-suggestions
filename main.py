...
from clock import trading_date_ist, now_ist
...
from analytics_mongo import MongoAnalyticsStore, InMemoryAnalyticsStore
from trade_journal import build_trade_journal_entry
from candle_recorder import candle_recorder_worker
...
def agent_loop_worker(
    ...
    while not stop_event.is_set():
        # Kill switch — file sentinel check
...
def main() -> None:
    ...
    trade_plan = cfg.security_id_map
    if not trade_plan:
    ...
    logger.info(f"Loaded {len(trade_plan)} stocks for autonomous monitoring.")
